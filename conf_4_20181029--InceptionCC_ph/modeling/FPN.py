...
## Inception lateral - using topdown
    ...
    output_blobs[num_backbone_stages - 1], _ = model.net.Concat(
        ...
        ['fpn_pre_{}'.format(fpn_level_info.blobs[num_backbone_stages - 1]),'fpn_bot_dim'],
        order = 'NCHW'
    )
    ...
    for j in range(num_backbone_stages - 1):
        ...
        output_blobs[num_backbone_stages - 2 - j], _ = model.net.Concat(
            ...
            ['fpn_pre_{}'.format(fpn_level_info.blobs[num_backbone_stages - 2 - j]),'fpn_bot_dim' + str(num_backbone_stages - 2 - j)],
            order = 'NCHW'
        )
