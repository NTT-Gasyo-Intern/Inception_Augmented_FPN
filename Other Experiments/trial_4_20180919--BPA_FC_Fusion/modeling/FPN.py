...
def add_fpn(model, fpn_level_info):
    ...
    output_blobs_panet = [
        'fpn_inner_panet_{}'.format(s)
        for s in fpn_level_info.blobs[:num_backbone_stages]
    ]
    #output_blobs_downtop = [
    #    'fpn_inner_downtop_{}'.format(s)
    #    for s in fpn_level_info.blobs[:num_backbone_stages]
    #]
    #output_blobs_add = [
    #    'fpn_inner_add_{}'.format(s)
    #    for s in fpn_level_info.blobs[:num_backbone_stages]
    #]
    ...
    output_blobs_panet[3] = output_blobs[3]

    # PANet path
    for j in range(num_backbone_stages - 1):
        add_dt_lateral_module(
            model,
            output_blobs_panet[3-j],             # top-down blob
            output_blobs[2-j],  # lateral blob
            output_blobs_panet[2-j],         # next output blob
            fpn_dim                     # output dimension
        )
    #output_blobs_downtop[3] = lateral_input_blobs[3]

    # downup path
    #for k in range(num_backbone_stages - 1):
    #    add_downtop_lateral_module(
    #        model,
    #        output_blobs_downtop[3-k],
    #        lateral_input_blobs[2-k],
    #        output_blobs_downtop[2-k],
    #        fpn_dim,
    #        fpn_dim_lateral[2-k]
    #    )
        
    # path fusion
    #for l in range(num_backbone_stages):
    #    model.net.Sum([output_blobs[l], output_blobs_downtop[l]], output_blobs_add[l])
    #    model.net.Sum([output_blobs_add[l], output_blobs_panet[l]], output_blobs_add[l])
